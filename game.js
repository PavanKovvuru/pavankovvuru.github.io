...
function create() {
  ...
  this.physics.add.collider(bugs, walls, function (bug) {
   bug.destroy();
    ...
    bugGenLoop.destroy();
    this.physics.pause();
    this.add.text(750, 250, 'Game Over', { fontSize: '50px', fill: '#ffffff' });
    this.add.text(650, 350, 'Click to Restart', { fontSize: '50px', fill: '#ffffff' });
...
function update() {
  ...
  // Add your conditional statements below:
    ...
    gameState.player.setVelocityX(-1);
  }
