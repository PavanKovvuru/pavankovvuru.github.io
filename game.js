...
function create() {
  ...
  const walls = this.physics.add.staticGroup();
   ...
   walls.create(1850, 600, 'wall').setScale(.3, 1).refreshBody();
  ...
  //  game.world.setBounds(0, 0, 1920, 1920);
   ...
   // player = game.add.sprite(game.world.centerX, game.world.centerY, 'codey');
    gameState.scoreText = this.add.text(900, 700, 'Score: 0', { fontSize: '30px', fill: '#000000' });
  ...
  this.physics.add.collider(bugs, walls, function (bug) {
   bug.destroy();
    ...
    bugGenLoop.destroy();
    this.physics.pause();
    this.add.text(900, 250, 'Game Over', { fontSize: '50px', fill: '#ffffff' });
    this.add.text(850, 300, 'Click to Restart', { fontSize: '50px', fill: '#ffffff' });
